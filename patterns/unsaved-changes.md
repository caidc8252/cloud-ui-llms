...
Catch the user before a dirty form is thrown away. **Required on every create and edit form, in a page or an overlay.**

[Create form](create-form.md) | [Edit resource](edit-resource.md)

## Key UX concepts

### One dirty flag for the whole form

The same comparison — draft against the pristine record — drives the commit button's `disabled` state and every leave guard. Compute it once, by **value equality**, and derive all related behaviour from it. Two flags will disagree, and the day they do, the user loses work.
...
A form in a `Modal` or `Sheet` has three exits: the escape button, the close affordance, and `Esc`. Route every one through the same dismiss behaviour and have it consult the dirty flag.
...
### Guard every in-product exit you control

On a page, the user can leave through the header back action, breadcrumbs, sidebar navigation, an explicit Cancel action, or navigation started after another action. Every in-product exit must consult the same dirty flag and open the same leave dialog.

Hold the destination the user chose while the dialog is open. If they discard or save successfully, continue to that destination; if they keep editing, leave them in the form.

### Browser exits use browser feedback

Closing or reloading the browser tab may produce a generic, browser-authored prompt. Its wording and styling are outside the design system. Treat it as a fallback for leaving the product, not as a replacement for the in-product `AlertDialog`.
...
A value comparison of the draft against the pristine record. After a successful save, replace the pristine baseline with the values the form now displays.

#### B. Exit guard

One shared behaviour for the reduced header's back action, sidebar, breadcrumb, Cancel action, and every other in-product exit.

#### C. Leave dialog

`AlertDialog` with a `danger` _Discard changes_, an `AlertDialogCancel` carrying _Keep editing_, and — when the form is valid — a _Save and leave_. _Save and leave_ is asynchronous, so it is a plain `Button` with `loading`, not an `AlertDialogAction`; see [AlertDialog → Async confirm](../components/alert-dialog.md).

#### D. The pending destination
...
- Compute one dirty flag by value comparison, and drive the commit and every leave guard from it.
- After a successful save, update the pristine baseline so the form becomes clean.
...
- Route every in-product exit through the same dirty-state check.

### Don't

- Don't let the back action, breadcrumbs, sidebar, Cancel action, or automatic navigation bypass the guard.
- Don't treat a generic browser prompt as the in-product pattern.
