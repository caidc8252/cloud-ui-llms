...
`@cloud/ui` composes application pages from the [Layout source](../../src/components/layout/layout.tsx), [page header sources](../../src/components/layout/page-header.tsx), [PageHeaderBand source](../../src/components/layout/page-header-band.tsx), [PageBody source](../../src/components/layout/page-body.tsx), and [ActionFooter source](../../src/components/layout/action-footer.tsx). The maintained examples and the portal [UI and page rules](../../../../.claude/team-rule/coding-rules/ui_ui-and-pages.md) are the canonical page recipes.
...
When a sidebar is supplied, `Layout` renders it as a desktop `aside` from `md` upward and also renders it in a left [Sheet](../../src/components/ui/primitives/sheet.tsx) for mobile. Keep the shared sidebar in that shell and trigger the existing mobile drawer rather than creating separate page navigation trees.
