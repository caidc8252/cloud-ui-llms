...
[Source](https://github.com/Newland-Payment-Technology-US-Co-Ltd/cloud-next-scaffold/blob/develop/packages/ui/src/components/ui/recipes/pagination.tsx) | [Public exports](https://github.com/Newland-Payment-Technology-US-Co-Ltd/cloud-next-scaffold/blob/develop/packages/ui/src/components/ui/index.ts)

`Pagination` is a client component driven by props. Import it from `@cloud/ui` or `@cloud/ui/components/ui`.

## Development guidelines

`Pagination` is the bare pager: previous, the page numbers, next. It is fully controlled — `page` is 1-based, `pageCount` is the total, and `onChange` receives the new page number.
...
Most list and table footers want `RichPagination` instead: it wraps this pager with a rows-per-page selector and a "showing X–Y of Z" summary, localizes its own text, and collapses responsively. Reach for bare `Pagination` when you need only the page buttons.
...
- Don't use bare `Pagination` for a table footer; `RichPagination` gives you the range summary and page-size selector for free.
...
- The current page is not conveyed by its fill alone; it is announced as the current item.
